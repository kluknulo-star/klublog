...
@section('content')
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6 d-flex align-items-center">
                        <h1 class="m-0 mr-2">Пост №{{$post->post_id}}</h1>
                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-head-fixed text-nowrap">
                                    ...
                                    <tr>
                                        <td>{{$post->post_id}}</td>
                                        <td>{{$post->title}}</td>
                                        <td>{{$post->created_at?? 'Неизвестно'}} </td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
    ...
    <!-- Modal -->
    <div class="modal fade" id="deleteCategoryModal{{$post->post_id}}" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{route('admin.post.destroy', ['post' => $post->post_id])}}" method="post">
                        @csrf
                        @method('DELETE')
